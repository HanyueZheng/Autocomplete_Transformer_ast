...
from Train import run_epoch, greedy_decode, beam_search_decode, SimpleLossCompute, run_epoch_kg, beam_search_decode_kg, run_epoch_ast
...
def data_gen_token_ast(dataloader, batch, nbatches, chunk_len, device):
    "为src-tgt复制任务生成随机数据"
    for i in range(nbatches):
        src_tgt_pair = dataloader.next_chunk()
        data_src = src_tgt_pair[0].unsqueeze(0)
        data_ent = src_tgt_pair[2].unsqueeze(0)
        data_tgt = src_tgt_pair[1].unsqueeze(0)
        data_ast = src_tgt_pair[3].unsqueeze(0)
        for k in range(batch-1):
            src_tgt_pair = dataloader.next_chunk()
            data_src = torch.cat([data_src, src_tgt_pair[0].unsqueeze(0)])
            data_ent = torch.cat([data_ent, src_tgt_pair[2].unsqueeze(0)])
            data_tgt = torch.cat([data_tgt, src_tgt_pair[1].unsqueeze(0)])
            data_ast = torch.cat([data_ast, src_tgt_pair[3].unsqueeze(0)])
        src = Variable(data_src, requires_grad=False)
        ent = Variable(data_ent, requires_grad=False)
        tgt = Variable(data_tgt, requires_grad=False)
...
if __name__ == "__main__":
    ...
    sys.argv.append('transformer_kg_1000_20_9_3.model')
    sys.argv.append('In addition, when the train stopped, ATP sends the odmeter ')
    ...
    if (method == "train"):
        ...
        if is_char_level:
            ...
            for epoch in range(epoch_number):
                ...
                print("step: ", epoch)
                model.train()
                run_epoch_ast("train", data_gen_token_ast(dataloader_ast, batch, nbatches, chunk_len, device), model,
                          SimpleLossCompute(model.generator, criterion, model_opt), nbatches, epoch)
                model.eval()
                run_epoch_ast("test ", data_gen_token_ast(dataloader_ast, batch, nbatches, chunk_len, device), model,
                          SimpleLossCompute(model.generator, criterion, None), nbatches, epoch)
