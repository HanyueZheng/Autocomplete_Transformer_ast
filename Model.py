...
from Layers import EncoderLayer,DecoderLayer,clones, EncoderLayer4AST
...
def make_model_ast(ast_vocab, src_vocab, tgt_vocab, kg_embed, N=6, d_model=512, d_ff=2048, d_intermediate=512, h=8, dropout=0.1):
	"从超参数构造模型"
	c = copy.deepcopy
	attn = MultiHeadedAttention(h, d_model)
	attn_ent = MultiHeadedAttention(h, d_model)
	...
	model = EncoderDecoder4AST(
		Encoder4KG(EncoderLayer4KG(d_model, d_intermediate, c(attn), c(attn_ent), c(ff), dropout), N),
		Encoder4AST(EncoderLayer4AST(d_model, c(attn_ast), c(ff), dropout), N),
		Decoder(DecoderLayer(d_model, c(attn), c(attn), c(ff), dropout), N),
		nn.Sequential(Embeddings(d_model, src_vocab), c(position)),
		nn.Sequential(Embeddings(d_model, 9000), c(position)),
		torch.nn.Embedding.from_pretrained(embed),
		nn.Sequential(Embeddings(d_model, tgt_vocab), c(position)),
		Generator(d_model, tgt_vocab))
...
class EncoderDecoder4AST(nn.Module):
	...
	def __init__(self, encoder, encoder4ast, decoder, src_embed, ast_embed, ent_embed, tgt_embed, generator):
		super(EncoderDecoder4AST, self).__init__()
		self.encoder = encoder
		self.astencoder = encoder4ast
		self.decoder = decoder
		self.src_embed = src_embed
	...
	def forward(self, src, ast, ent, tgt, src_mask, ast_mask, ent_mask, tgt_mask):
		"处理屏蔽的源序列与目标序列"
		src.size()
		ent.size()
		tgt.size()
		ast.size()
		return self.decode(self.encode(src, src_mask, ast, ast_mask, ent, ent_mask), src_mask, tgt, tgt_mask)

	def encode(self, src, src_mask, ast, ast_mask, ent, ent_mask):
		print ("ast")
		print (ast)
		print ("absast")
		print (torch.FloatTensor.abs_(ast))
		self.ast_embed(ast)
		self.src_embed(src)
		print (self.ent_embed)
		self.ent_embed(ent)

		return self.encoder(self.src_embed(src), src_mask, self.ent_embed(ent), ent_mask) + self.astencoder(self.ast_embed(ast), ast_mask)
